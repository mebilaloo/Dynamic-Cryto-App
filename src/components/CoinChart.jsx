...
const CoinChart = ({ currency }) => {
  ...
  const myData = {
    ...
    datasets: [
      {
        label: `Price in Past ${days} Day(s) in ${currency}`,
        ...
        chartData.length === 0 ? (<Loader />) : (
          <div>
            ...
            <div className='btn' style={{ marginTop: "30px" }}>
              <button style={{padding:'3px'}} onClick={() => setDays(1)}>24 Hours</button>
              <button style={{padding:'3px'}} onClick={() => setDays(30)}>1 Month</button>
              <button style={{padding:'3px'}} onClick={() => setDays(365)}>1 Year</button>
            </div>
          </div>
        )
      }
